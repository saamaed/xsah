...
The first software that runs after a machine is powered on is its [**firmware**](https://www.hp.com/hk-en/tech-takes/software/explainer/what-is-uefi.html).
...
This is the [**filesystem**](/tags/filesystem/) that contains the Linux userspace hierarchy discussed in the earlier filesystem post.
...
Once `systemd` starts as PID 1, it becomes responsible for bringing the system into its configured operational state. It starts and manages services, mounts filesystems, creates runtime resources, and coordinates other parts of userspace initialization.
...
Understanding boot is useful far beyond troubleshooting a desktop Linux installation. Servers depend on the same fundamental layers to become operational after a restart.
